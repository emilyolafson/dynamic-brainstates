% Script to plot flux map for Ising data. Use 'fluxMap_Ising_bootstrap'
% first to calculate the flux map.

% Colormap:
% color = .2*[1 1 1] + .8*[0 77 128]/255; % For T = 0.1
% color = .8*(.3*[1 1 1] + .7*[148 33 147]/255); % For T = 1
color = .2*[1 1 1] + .8*.8*[181 23 0]/255; % For T = 10
X = linspace(1, 0, 1000)';
...
% Units to measure current (in inverse trials):
units = 5*10^(-3);

% Number of standard deviations and alpha (translucency) for ellipses:
% num_stds = 1;
num_stds = 2;
std_alpha = .25;
...
for i = 1:num_bins
    ...
    C = cov(reshape(current_temp(1,i,:), 1, num_samples), reshape(current_temp(2,i,:), 1, num_samples));
    [V, D] = eigs(C);
...
fill([bin_pos(1,2) - .5*bin_size(1), bin_pos(1,3) + .5*bin_size(1), bin_pos(1,3) + .5*bin_size(1), bin_pos(1,2) - .5*bin_size(1)],...
    [bin_pos(2, end-2) - .5*bin_size(2), bin_pos(2, end-2) - .5*bin_size(2), bin_pos(2, end-1) + .5*bin_size(2), bin_pos(2, end-1) + .5*bin_size(2)],...
    'w', 'EdgeAlpha', 0);
quiver((bin_pos(1,2) - .25*bin_size(1))*[1 1], (bin_pos(2,end-2) - .35*bin_size(2))*[1 1],...
